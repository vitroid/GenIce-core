...
def noodlize(g: nx.Graph, fixed: Union[nx.DiGraph, None] = nx.DiGraph()) -> nx.Graph:
    ...
    for v in g:
        if fixg.has_node(v):
            ...
            nfixed = 0
        divide_node(divg, v, offset, fixed, nfixed)
...
def undecorate(g: Union[nx.Graph, nx.DiGraph]):
...
def balance(fixed: nx.DiGraph, g: nx.Graph, hook=None):
    ...
    perimeter = [
        node for node in fixed if fixed.in_degree[node] + fixed.out_degree[node] < 4
    ]
    ...
    while len(perimeter) > 0:
        ...
        while fixed.in_degree(node) > fixed.out_degree(node):
            e = random.choice(list(g[node]))
            if not (fixed.has_edge(node, e) or fixed.has_edge(e, node)):
                fixed.add_edge(node, e)
                perimeter.append(e)
        while fixed.in_degree(node) < fixed.out_degree(node):
            e = random.choice(list(g[node]))
            if not (fixed.has_edge(node, e) or fixed.has_edge(e, node)):
                fixed.add_edge(e, node)
                perimeter.append(e)
